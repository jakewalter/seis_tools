...
#`ls -1 $files/*/*H*$year.$fday > $put/F2`;
`ls -1 $files/*/$year.$fday*SAC > $put/F2`;
...
while ($i < @tablea) { 
	...
	($year,$day,$field3,$field4,$field5,$field6,$field7,$chan,) = split "\\.", $file1;
...
END
`;
print "$loc check thissssssssssssssssssssssssss222222222222";
...
`;
#`rm $loc`;
print "$loc check thissssssssssssssssssssssssss3333333333333333";
