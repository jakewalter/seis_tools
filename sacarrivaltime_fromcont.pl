...
if ( $ARGV[4] ){ $stdate = $ARGV[4]; }
if ( $ARGV[5] ){ $enddate = $ARGV[5]; }
...
#@dbo = dbsubset(@dbo,"orid>11096");    #   12573
...
for ( $dbe[3] = 0; $dbe[3] < $nrecords; $dbe[3]++ ) {
	...
	#print "$ofractionday $oall $ofractionday1 $ofractionday2 $ofractionday3 NNNN\n";
	...
	open(FILEOUT2, ">" , "$oall/wf_SNR.dat")  or die "Can't open wf_SNR.dat: $!";
	open(FILEOUT4, ">" , "$oall/phases.dat")  or die "Can't open phases.dat: $!";
	open(FILEOUT5, ">" , "$oall/origid.dat")  or die "Can't open catalog file: $!";
...
#`saclst o f $outdir/$oall/*.bp | gawk '{printf "r %s\nch allt %.5f\nwh\n",$1,$2*(-1)} END{print "quit"}' | sac`;
