...
cat $inputcatdir/15times* > 15timesall1.cat
...
#	perl /home/jwalter/bin/db2hyposingle.pl /data3/mendenhall/nicoyaclean/nicclean hypoddnewevent $oldorigid $neworigid
	...
	#yearmonthday=`gawk '{if(NR=='"$counter"') printf"%s%s%s\n",substr($0,1,4),substr($0,5,2),substr($0,7,2)}' $cont`
        templat=`gawk '{if(NR=='"$counter"') print $13}' $input`
	templon=`gawk '{if(NR=='"$counter"') print $14}' $input`
	tempdepth=`gawk '{if(NR=='"$counter"') print $15}' $input`
	...
	perl ~/bin/db2hyposingle.pl $originaldatabase hypoddnewevent $oldorigid $neworigid $year $month $day $hr $min $sec $inputcatdir
